...
const initialStatesList: StateRecord[] = [
...
const initialDistrictsList: DistrictRecord[] = [
...
const initialRolesList: RoleRecord[] = [
...
const masterDataSlice = createSlice({
  ...
  reducers: {
    // States CRUD
    addState: (state, action: PayloadAction<Omit<StateRecord, 'id'>>) => {
      const newId = state.states.length > 0 ? Math.max(...state.states.map(s => s.id)) + 1 : 1;
      state.states.push({ id: newId, ...action.payload });
    },
    updateState: (state, action: PayloadAction<Partial<StateRecord> & { id: number }>) => {
    ...
    // Districts CRUD
    addDistrict: (state, action: PayloadAction<Omit<DistrictRecord, 'id'>>) => {
      const newId = state.districts.length > 0 ? Math.max(...state.districts.map(d => d.id)) + 1 : 1;
      state.districts.push({ id: newId, ...action.payload });
    },
    updateDistrict: (state, action: PayloadAction<Partial<DistrictRecord> & { id: number }>) => {
    ...
    // Roles CRUD
    addRole: (state, action: PayloadAction<Omit<RoleRecord, 'id'>>) => {
      const newId = state.roles.length > 0 ? Math.max(...state.roles.map(r => r.id)) + 1 : 1;
      state.roles.push({ id: newId, ...action.payload });
    },
    updateRole: (state, action: PayloadAction<Partial<RoleRecord> & { id: number }>) => {
